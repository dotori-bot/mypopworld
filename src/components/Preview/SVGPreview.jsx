...
import { createSVG, svgToString, getLineStyle, addPath, addRect, addText, paintCardSkin } from '../../generators/svgBuilder';
...
export default function SVGPreview() {
  const { cardParams, isTyping, paperSize, colorMode, language, decorationMode, isGenerating, setGenerating, appMode, userArt, cardSkin } = useCardStore();
  ...
  useEffect(() => {
    ...
    // Card-skin (꾸미기 배경) painted under every pattern page's marks.
    pageEls.forEach((el) => paintCardSkin(el, paper, cardSkin));
    setMechPages(pageEls.map((el) => ({ el, str: svgToString(el) })));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cardParams, paperSize, colorMode, cardSkin]);
  ...
  return (
    <div className="preview-content">
      <div className="decoration-mode-toggle">
        <span className="decoration-mode-label">꾸미기:</span>
        <CustomizePanel />
        <span className="customize-summary">
          {decorationMode === 'ai-image' ? '장식: AI 이미지' : decorationMode === 'user-image' ? '장식: 내 그림' : '장식: 직접 그리기'}
          {cardSkin.type !== 'none' && ` · 배경: ${cardSkin.type === 'color' ? '단색' : '내 이미지'}`}
        </span>
      </div>
