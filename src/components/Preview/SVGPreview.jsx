...
export default function SVGPreview() {
  const { cardParams, isTyping, paperSize, colorMode, language, decorationMode, setDecorationMode, isGenerating, setGenerating, appMode, userArt, setCardParams, setChatDraft, setAppMode } = useCardStore();
  // Whether the 🔧 wrench "part edit" markers are shown over the design.
  ...
  // The element whose design is on the currently visible page (mechanism pages
  // come first, one per element; assembly-map/decoration pages have no element).
  const _els = getElements(cardParams);
  const partEl = currentPage < _els.length ? _els[currentPage] : null;

  // Merge a param patch into the visible page's element (v1 top-level params or
  // v2 elements[]), so a wrench edit re-renders the design immediately.
  const commitPartPatch = (patch) => {
    const els = getElements(cardParams);
    if (currentPage >= els.length) return;
    if (!isMultiElement(cardParams)) {
      setCardParams({ ...cardParams, params: { ...(cardParams.params || {}), ...patch } });
    } else {
      setCardParams(
        withElements(
          cardParams,
          els.map((el, i) => (i === currentPage ? { ...el, params: { ...(el.params || {}), ...patch } } : el)),
        ),
      );
    }
  };

  // Hand a scoped question about the clicked part to the chat and switch to
  // kids (chat) mode so the user can discuss the change conversationally.
  const discussPart = (part) => {
    const el = getElements(cardParams)[currentPage];
    const mech = getMechanism(el?.mechanism);
    const name = mech?.labelKo || el?.mechanism || '이 도안';
    const subject = cardParams?.theme ? `'${cardParams.theme}' 카드(${name})` : name;
    setChatDraft(
      `${subject}에서 '${part.labelKo}' 부분을 바꾸고 싶어요. ${part.tip || ''} 어떻게 바꾸면 좋을지 알려주세요.`.trim(),
    );
    setAppMode('kids');
  };
    ...
    <div className="preview-content">
      <div className="decoration-mode-toggle">
        ...
        {partEl && (
          <button
            type="button"
            className={`part-edit-toggle ${showParts ? 'is-active' : ''}`}
            aria-pressed={showParts}
            title="도안에서 바꿀 수 있는 부분에 🔧 표시를 켜고 끕니다"
            onClick={() => setShowParts((v) => !v)}
            style={{ marginLeft: 'auto' }}
          >
            <Wrench size={14} /> 부분 편집 {showParts ? '켜짐' : '꺼짐'}
          </button>
        )}
      </div>
          ...
          <div className="svg-paper" style={{ width: `${paper.width}mm`, height: `${paper.height}mm`, position: 'relative' }}>
            ...
            {showParts && partEl && (
              <PartEditOverlay
                ...
                colorMode={colorMode}
                theme={cardParams?.theme}
                onPatch={commitPartPatch}
                onDiscuss={discussPart}
              />
            )}
          </div>
        )}
