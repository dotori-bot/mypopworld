...
export default function SVGPreview() {
  const { cardParams, isTyping, paperSize, colorMode, language, decorationMode, setDecorationMode, isGenerating, setGenerating, appMode, userArts } = useCardStore();
  const hasAnyArt = Object.values(userArts).some(Boolean);
  ...
  useEffect(() => {
    ...
    // 'user-image' needs at least one uploaded drawing to work with; without
    // any, fall back to the freehand guide (the mode button is disabled
    // anyway, this guards races where the last art is cleared while the mode
    // is still set). Slots WITHOUT their own upload keep the freehand size
    // guide per slot — see slotMode in the loop below.
    const anyArt = Object.values(userArts).some(Boolean);
    const effectiveMode = decorationMode === 'user-image' && !anyArt ? 'freehand' : decorationMode;
    ...
    // (prompt, theme, mode, paper, real size changes) does. The per-slot art
    // hashes only participate while they're actually shown, so re-uploading
    // a drawing never invalidates AI/freehand pages.
    const decoKey = JSON.stringify({
      ...
      userArts:
        effectiveMode === 'user-image'
          ? slots.map((_, i) => (userArts[i] ? `${userArts[i].length}:${userArts[i].slice(-24)}` : 0))
          : null,
      paperSize,
      colorMode,
    });
    ...
    const run = async () => {
      ...
      const decorationPages = await Promise.all(slots.map(async (slot, i) => {
        ...
        // Per-slot mode: in 'user-image' every slot with its OWN uploaded
        // drawing embeds that drawing; slots the user hasn't filled yet keep
        // the freehand guide (which announces the slot's size/position), so
        // the child still knows what to draw/upload for the missing spots.
        const slotArt = userArts[i];
        const slotMode = effectiveMode === 'user-image' ? (slotArt ? 'user-image' : 'freehand') : effectiveMode;

        if (slotMode === 'user-image') {
          ...
          const contourPath = await getContourPath(slotArt, artX, artY, artW, artH, 5);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cardParams, paperSize, colorMode, decorationMode, userArts]);
    ...
    <div className="preview-content">
      <div className="decoration-mode-toggle">
        ...
        <button
          ...
          disabled={!hasAnyArt}
          title={hasAnyArt ? undefined : '먼저 아래 슬롯에 그림을 업로드하세요'}
