...
export const INSTRUCTION_TEXT = {
  'v-fold': {
    ...
    steps: [
      '검은색 실선을 따라 다이아몬드(마름모) 모양 팝업 조각을 오려주세요. 위·아래 끝에 붙은 초록색 날개(풀칠 자리)는 자르지 말고 남겨둡니다.',
      '척추와 겹치는 가운데 가로 능선(빨간 점선)은 산접기(볼록하게) 해서 앞으로 뾰족하게 서게 하고, 위·아래 두 바닥선(파란 점선)은 골접기(오목하게) 해서 두 팔이 각각 반대쪽 종이면으로 눕게 접어주세요.',
      '카드를 반쯤 펼친 상태에서 능선을 척추(가운데 접는 선) 위에 정확히 얹고, 위쪽 날개는 위 종이면에 아래쪽 날개는 아래 종이면에 척추를 기준으로 상하 대칭이 되도록 붙여주세요.',
    ],
    tips: '카드를 닫으면 다이아몬드가 척추를 따라 납작하게 접히고, 열면 능선이 앞으로 솟아올라야 정상입니다. 한쪽으로 기울면 두 날개를 척추 기준으로 대칭이 되게 다시 붙여 주세요.',
  },
  generic: {
    title: '조립 설명서',
    materials: '가위, 풀 또는 양면테이프, 색연필(선택)',
