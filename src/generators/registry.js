...
export const INSTRUCTION_TEXT = {
  ...
  'magic-shutter': {
    ...
    steps: [
      ...
      '슬라이더를 앞면 카드 뒤에 대고, 손잡이를 카드 오른쪽 밖으로 빼내세요. 창문 전체가 슬라이더로 덮이는지 확인합니다.',
      '중요(안내·멈춤 다리): 위 안내 다리를 슬라이더 위쪽에 다리처럼 얹어 풀칠 면(초록)만 카드에 붙이고, 접는 선의 립(날개)을 슬라이더 쪽으로 접어 덮어주세요 — 립은 슬라이더에 붙이면 안 됩니다! 아래 안내 다리도 같은 방법으로 슬라이더 아래쪽에 붙이되, 가운데 멈춤 핀을 산접기해서 슬라이더의 멈춤 슬롯 안에 끼워주세요. 이 핀이 슬롯 양 끝에 걸려 손잡이가 딱 한 칸만 움직입니다.',
