...
- 고정 반원의 지름선에는 이제 SCORE 선을 긋지 않는다 — 그 자리는 조각의 절단 외곽선(그리고 GLUE_TAB 사각형의 오른쪽 경계)과 정확히 같은 좌표라서, 회색 선을 추가하면 절단선 위에 이중 등록될 뿐이었다. 넘김판의 지름선에 긋는 VALLEY_FOLD(파란 점선)만이 실제로 접히는 힌지다. 향후 접기 방향 검증 도구를 추가한다면 고정 반원은 "접히는 선이 없는 조각"으로 취급할 것.
