...
`generateLayeredStage`: 윗 반쪽(뒷벽 면)에 뒷날개 풀칠선 ①..Ⓝ(SCORE)와 **1층 띠**, 척추 아래(바닥 면)에 1층 아래날개 풀칠선 ㉠과 **2..N층 띠**(한 줄에 최대 2장 flow 배치). 각 띠는 4변 모두 절단(완전 분리 부품), 내부 크리스는 골–산–골, 날개는 GLUE 스타일. **위층의 아래날개 풀칠선(㉡)은 아래층 띠의 윗면 패널 위에 인쇄**되어 접착 위치 오차가 누적되지 않게 한다. 앞면 패널에는 `drawFacade` 장식 SCORE 가이드.
